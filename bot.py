...
BOT_TOKEN = "8623563609:AAHst1gExAr9kFdPiQuUluDP-KN5EosEmbE"
ADMIN_ID = [8772814136]
GROUP_ID = -1003154512032
...
async def send_hit_notification(client_instance, card, result, username, user_id):
    try:
        ...
        hit_msg = f"""{emoji} 𝗛𝗜𝗧 𝗗𝗘𝗧𝗘𝗖𝗧𝗘𝗗 {emoji}
━━━━━━━━━━━━━━━━━━━
𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲 ┃ {response}
𝗚𝗮𝘁𝗲𝘄𝗮𝘆 ┃ {gateway}
𝗣𝗿𝗶𝗰𝗲 ┃ {price}
...
def access_denied_message_with_button():
    message = "⛔ **𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱** ━ Premium or group usage required."
    buttons = [[Button.url("◈ Join Group", "https://t.me/ArxisShopify")]]
...
async def start(event):
    ...
    text = """◈ **𝗦𝗛𝗢𝗣𝗜𝗙𝗬 𝗖𝗛𝗘𝗖𝗞𝗘𝗥** ◈
━━━━━━━━━━━━━━━━━━━

✦ **𝗖𝗵𝗲𝗰𝗸𝗲𝗿 𝗖𝗼𝗺𝗺𝗮𝗻𝗱𝘀**
`/sh` ━ Single CC check
`/msh` ━ Multi CC from text
`/mtxt` ━ Mass CC from `.txt` file
`/ran` ━ Mass CC with random sites

✦ **𝗠𝗮𝗻𝗮𝗴𝗲𝗺𝗲𝗻𝘁**
`/add` ━ Add site(s) to your DB
`/rm` ━ Remove site(s) from DB
`/check` ━ Test saved sites
`/info` ━ Your profile & stats
`/redeem` ━ Redeem a premium key

✦ **𝗣𝗿𝗼𝘅𝘆** (Private Only)
`/addpxy` ━ Add proxy (max 10)
`/proxy` ━ View saved proxies
`/chkpxy` ━ Test proxy status
`/rmpxy` ━ Remove proxy
━━━━━━━━━━━━━━━━━━━
"""

    if access_type in ["premium_private", "premium_group"]:
        text += f"◈ **𝗦𝘁𝗮𝘁𝘂𝘀** ━ Premium ◈ (`{get_cc_limit(access_type, event.sender_id)}` CCs)"
    else:
        text += f"▰ **𝗦𝘁𝗮𝘁𝘂𝘀** ━ Free Tier ▰ (`{get_cc_limit(access_type, event.sender_id)}` CCs)"
...
async def generate_keys(event):
    ...
    try:
        ...
        days = int(parts[2])
        if amount > 10:
            return await event.reply("✗ Max 10 keys at once")
        generated_keys = []
...
async def redeem_key_cmd(event):
    ...
    try:
        ...
        if await is_premium_user(event.sender_id):
            return await event.reply("▰ You already have premium")
...
async def add_site(event):
    ...
    try:
        ...
        response_parts = []
        if added_sites:
            response_parts.append("\n".join(f"✦ Added: {s}" for s in added_sites))
        if already_exists:
            response_parts.append("\n".join(f"⚠ Exists: {s}" for s in already_exists))
        if response_parts:
            await event.reply("\n\n".join(response_parts))
        else:
...
async def remove_site(event):
    ...
    try:
        ...
        if response_parts:
            ...
            await event.reply("✗ No sites removed")
    except Exception as e:
        await event.reply(f"✗ Error: {e}")
...
async def add_proxy_cmd(event):
    ...
    try:
        ...
        if event.is_reply:
            reply_msg = await event.get_reply_message()("Format: /addpxy ip:port:username:password\n")
            ...
            # If text message
                ...
                proxy_lines = reply_msg.text.splitlines()

        # Case 2: User typed proxies in command
        else:
            parts = event.raw_text.split(maxsplit=1)
            if len(parts) == 2:
                proxy_lines = parts[1].splitlines()
            else:
                return await event.reply(
                    "Usage:\n"
                    "/addpxy ip:port:user:pass\n"
                    "OR send multiple proxies\n"
        ...
        for existing_proxy in existing_proxies:
            if existing_proxy['proxy_url'] == proxy_data['proxy_url']:
                return await event.reply("⚠ Proxy already added")
        ...
        if not is_working:
            await testing_msg.edit(f"✗ **Proxy not working**\n\n┃ {result}")
            return
...
async def remove_proxy_cmd(event):
    ...
    try:
        ...
        if not user_proxies:
            return await event.reply("✗ No proxies saved")
        ...
        if arg == 'all':
            ...
            return await event.reply(f"✦ All {count} proxies removed")
...
async def view_proxy(event):
    ...
    try:
        ...
        proxy_list = f"◈ **𝗣𝗿𝗼𝘅𝗶𝗲𝘀** ({len(user_proxies)}/10)\n━━━━━━━━━━━━━━━━━━━\n"
        ...
        proxy_list += f"\n━━━━━━━━━━━━━━━━━━━\n▰ Random selection per check\n▰ `/rmpxy <index>` to remove\n▰ `/rmpxy all` to clear"
...
async def check_proxy_cmd(event):
    ...
    try:
        ...
        status_msg = await event.reply(f"```◈ Testing {len(user_proxies)} proxies...```")
        ...
        for idx, proxy_data in enumerate(user_proxies, 1):
            ...
            if is_working:
                working.append(f"✦ `{idx}.` {proxy_type} ┃ {display} → {result_ip}")
            else:
                dead.append(f"✗ `{idx}.` {proxy_type} ┃ {display}")

            try:
                await status_msg.edit(f"```◈ Testing proxies ━ {idx}/{len(user_proxies)}```")
            except Exception:
                pass

        result_text = f"◈ **𝗣𝗿𝗼𝘅𝘆 𝗖𝗵𝗲𝗰𝗸**\n━━━━━━━━━━━━━━━━━━━\n"
        ...
        result_text += f"━━━━━━━━━━━━━━━━━━━\n✦ {len(working)} working ┃ ✗ {len(dead)} dead ┃ ▰ {len(user_proxies)} total"

        if dead:
            result_text += "\n\n▰ Use `/rmpxy <index>` to remove dead proxies"
...
async def sh(event):
    ...
    if access_type == "banned":
        return await event.reply(banned_user_message())
    if not can_access:
        buttons = [[Button.url("◈ Join Group", f"https://t.me/ArxisShopify")]]
        return await event.reply("⛔ **𝗨𝗻𝗮𝘂𝘁𝗵𝗼𝗿𝗶𝘀𝗲𝗱**\n\nUse in group for free or get premium.\n◈ Contact @ArxisX", buttons=buttons)
    await ensure_user(event.sender_id)
...
async def process_sh_card(event, access_type):
    ...
    if not proxy_data:
        return await event.reply("⚠ **𝗣𝗿𝗼𝘅𝘆 𝗥𝗲𝗾𝘂𝗶𝗿𝗲𝗱**\n━━━━━━━━━━━━━━━\nAdd one first:\n`/addpxy ip:port:user:pass`")
    ...
    if event.reply_to_msg_id:
        ...
        if not card:
            return await event.reply("━━ 𝗜𝗻𝘃𝗮𝗹𝗶𝗱 𝗖𝗮𝗿𝗱 ━━\n\nFormat ➜ `/sh 4111111111111111|12|2025|123`")
    else:
        card = extract_card(event.raw_text)
        if not card:
            return await event.reply("━━ 𝗙𝗼𝗿𝗺𝗮𝘁 ━━\n\n`/sh 4111111111111111|12|2025|123`\n\nOr reply to a message with CC info", parse_mode="markdown")

    user_sites = await get_user_sites(event.sender_id)
    if not user_sites:
        return await event.reply("⚠ No sites found. Add with `/add`")
...
async def msh(event):
    ...
    if event.reply_to_msg_id:
        ...
        if not cards:
            return await event.reply("━━ 𝗜𝗻𝘃𝗮𝗹𝗶𝗱 ━━\n\nFormat: `/msh CC|MM|YY|CVV CC|MM|YY|CVV`")
    else:
        cards = extract_all_cards(event.raw_text)
        if not cards:
            return await event.reply("━━ 𝗙𝗼𝗿𝗺𝗮𝘁 ━━\n\n`/msh CC|MM|YY|CVV CC|MM|YY|CVV`\n\nOr reply to a message with multiple cards")
    ...
    pref_msg = await event.reply(
        "◈ **𝗦𝗲𝗹𝗲𝗰𝘁 𝗠𝗼𝗱𝗲**\n━━━━━━━━━━━━━━━\n"
        "_✦ Yes: Charged + Approved cards_\n"
        "_◈ No: Only Charged cards_",
        buttons=buttons
    )
...
async def process_msh_cards(event, cards, sites, send_approved=True):
    ...
    sent_msg = await event.reply(f"```◈ Processing ━ {len(cards)} cards ━ {mode_text}```")
...
"""
    ...
    await sent_msg.edit(f"```◈ Mass Check Complete ━ {len(cards)} cards processed```")
...
async def mtxt(event):
    ...
    if user_id in ACTIVE_MTXT_PROCESSES:
        return await event.reply("```◈ Already processing ━ wait for completion```")
    try:
        if not event.reply_to_msg_id:
            return await event.reply("```━━ Reply to a .txt file with /mtxt```")
        replied_msg = await event.get_reply_message()
        ...
        if not cards:
            return await event.reply("━━ No valid CCs found ━━")
        cc_limit = get_cc_limit(access_type, user_id)
        total_cards_found = len(cards)
        if len(cards) > cc_limit:
            cards = cards[:cc_limit]
            await event.reply(f"""```◈ Found {total_cards_found} CCs
━ Limit: {cc_limit} CCs
✦ Checking {len(cards)} CCs```""")
        else:
            await event.reply(f"""```◈ Found {total_cards_found} valid CCs
✦ Checking all {len(cards)} CCs```""")
...
async def process_mtxt_cards(event, cards, local_sites, send_approved=True):
    ...
    is_private = event.chat.id == event.sender_id
    mode_text = "Charged + Approved" if send_approved else "Only Charged"
    status_msg = await event.reply(f"```◈ Processing ━ {mode_text}```")
    current_site_index = 0
    BATCH_SIZE = 20
    last_card_display = ""
    ...
    try:
        ...
        while idx < total:
            if not local_sites:
                await status_msg.edit("━━ **All sites dead** ━━\nAdd fresh sites with `/add`")
                break

            if user_id not in ACTIVE_MTXT_PROCESSES:
                final_caption = f"""⛔ 𝗦𝘁𝗼𝗽𝗽𝗲𝗱
━━━━━━━━━━━━━━━━━━
◈ Charged ┃ {charged}
✦ Approved ┃ {approved}
✗ Declined ┃ {declined}
...
"""
                final_buttons = [
                    [Button.inline(f"◈ Charged ┃ {charged}", b"none")],
                    [Button.inline(f"✦ Approved ┃ {approved}", b"none")],
                    [Button.inline(f"⛔ Stopped ┃ {checked}/{total}", b"none")]
                ]
                try:
            ...
            for i, (card, result) in enumerate(zip(batch, results)):
                ...
                if status in ["SiteError", "Error"]:
                    ...
                    if not local_sites:
                        final_caption = f"""━━ **All sites dead** ━━
Add fresh sites with `/add`
...
"""
                        final_buttons = [
                            [Button.inline(f"◈ Charged ┃ {charged}", b"none")],
                            [Button.inline(f"✦ Approved ┃ {approved}", b"none")],
                            [Button.inline(f"⛔ Sites Dead ┃ {checked}/{total}", b"none")]
                        ]
                        try:
                            await status_msg.edit(final_caption, buttons=final_buttons)
                        except Exception:
...
"""
            ...
            buttons = [
                [Button.inline(f"▰ {last_card_display}", b"none")],
                [Button.inline(f"┃ {last_response_display}", b"none")],
                [Button.inline(f"◈ Site ┃ {last_site_display}", b"none")],
                [Button.inline(f"◈ Charged ┃ {charged}", b"none")],
                [Button.inline(f"✦ Approved ┃ {approved}", b"none")],
                [Button.inline(f"✗ Declined ┃ {declined}", b"none")],
                [Button.inline(f"⚠ Errors ┃ {errors}", b"none")],
                [Button.inline(f"▰ {checked}/{total}", b"none")],
                [Button.inline("⛔ Stop", f"stop_mtxt:{user_id}".encode())]
            ]
            try:
                await status_msg.edit("```◈ Processing batches of 20...```", buttons=buttons)
            except Exception:
                pass

            idx += len(batch)

        final_caption = f"""◈ **𝗖𝗼𝗺𝗽𝗹𝗲𝘁𝗲**
━━━━━━━━━━━━━━━━━━
◈ Charged ┃ {charged}
✦ Approved ┃ {approved}
✗ Declined ┃ {declined}
...
"""
        final_buttons = [
            [Button.inline(f"◈ Charged ┃ {charged}", b"none")],
            [Button.inline(f"✦ Approved ┃ {approved}", b"none")],
            [Button.inline(f"▰ Total ┃ {total}", b"none")],
            [Button.inline(f"▰ Checked ┃ {checked}/{total}", b"none")]
        ]
        try:
            await status_msg.edit(final_caption, buttons=final_buttons)
        except Exception:
...
async def info(event):
    ...
    premium_status = "◈ Premium" if has_premium else "▰ Free"
...
𝗨𝘀𝗲𝗿 ┃ {username}
𝗜𝗗 ┃ `{user_id}`
𝗔𝗰𝗰𝗲𝘀𝘀 ┃ {premium_status}
━━━━━━━━━━━━━━━━━━━
✦ **𝗦𝗶𝘁𝗲𝘀** ({len(user_sites)}):
{sites_text}
"""
...
async def stats(event):
    ...
    try:
        ...
        try:
            ...
            await event.reply("◈ Statistics report generated", file=stats_filename)
        finally:
            try:
...
async def check_sites(event):
    ...
    if not can_access:
        buttons = [[Button.url("Use In Group Free", f"https://t.me/ArxisShopify")]]
...
And if you want to check your DB sites and add working & remove not working sites, click below button:"""
    ...
    if len(sites_to_check) > 10:
        ...
        await event.reply(f"```⚠ Found {total_sites_found} sites ━ checking first 10```")
...
async def process_db_site_check(event, user_sites):
    ...
    status_text = f"```◈ Checking {total_sites} DB sites...```"
...
▰ Checked ┃ {checked}/{total_sites}"""
            ...
            status_text = f"""```◈ Checking DB Sites...
...
┃ Status: {result['status'].upper()}```"""
    ...
    final_text = f"""◈ **DB Check Complete**
━━━━━━━━━━━━━━━━━━━
✦ Working ┃ {len(working_sites)}
✗ Dead (Removed) ┃ {len(dead_sites)}
...
async def add_working_sites_callback(event):
    try:
        ...
        response_parts = []
        if added_sites:
            added_text = f"✦ **Added {len(added_sites)} Sites:**\n"
            for site in added_sites:
                added_text += f"┃ `{site}`\n"
            response_parts.append(added_text)

        if already_exists:
            exists_text = f"⚠ **{len(already_exists)} Already Exist:**\n"
        ...
        if response_parts:
            response_text = "\n".join(response_parts)
            response_text += f"\n━━━━━━━━━━━━━━━━━━━\n▰ Total DB Sites: {len(all_user_sites)}"
        else:
            response_text = "▰ All sites already in DB"
