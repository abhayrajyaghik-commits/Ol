...
PE = "⭐"   # MUST be real emoji (never use "?")
...
def _build_entities(html_text, emoji_ids=None):
    """
    Parse HTML and add custom emoji entities at PE positions.
    Fully safe + UTF-16 correct + overflow protected.
    """
    text, entities = thtml.parse(html_text)

    if emoji_ids:
        # ✅ safety: prevent extra emoji crash
        emoji_ids = emoji_ids[:text.count(PE)]
            ...
            if ch == PE:
                if idx < len(emoji_ids):
                    entities.append(MessageEntityCustomEmoji(
                        offset=utf16_pos,
                        length=1,
                        document_id=emoji_ids[idx]
                    ))
                    idx += 1

            # ✅ UTF-16 safe increment
