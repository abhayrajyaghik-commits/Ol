...
{status_line}"""
        ...
        kb = [
            [pbtn("?? Plans", data="plans"), pbtn("?? Support", url="https://t.me/ArxisX")],
        ]
        start_emojis = [
            CE["crown"], CE["crown"],
            CE["bolt"],
...
? ?? 500 CCs ? ?? Private Chat ? ?? Proxy
???????????????????
{PE} Contact @ArxisX to purchase"""

    kb = [[pbtn("?? Buy Now", url="https://t.me/ArxisX"), pbtn("?? Back", data="back_start")]]
...
async def sh(event):
    ...
    if access_type == "banned":
        ...
        return await styled_reply(event, ban_text, emoji_ids=ban_emojis)
    if not can_access:
        kb = [[pbtn("? Join Group", url="https://t.me/ArxisShopify")]]
        await styled_reply(event, "? <b>????????????????????????</b>\n\nUse in group for free or get premium.\n? Contact @ArxisX", kb, emoji_ids=[CE["stop"]])
        return
    await ensure_user(event.sender_id)
...
async def check_sites(event):
    ...
    if not can_access:
        kb = [[pbtn("Use In Group Free", url="https://t.me/ArxisShopify")]]
        await styled_reply(event, f"{PE} <b>Unauthorised Access!</b>\n\nYou can use this bot in group for free!\n\nFor private access, contact @ArxisX", kb, emoji_ids=[CE["stop"]])
        return
