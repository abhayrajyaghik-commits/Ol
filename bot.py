...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8623563609:AAFwfKbkitCHUEW7WlulGgksBAIRIbz_ZCQ")
ADMIN_ID = json.loads(os.getenv("ADMIN_ID", "[6598607558]"))
GROUP_ID = int(os.getenv("GROUP_ID", "-1003684602999"))

# API Base URL
API_BASE_URL = os.getenv("API_BASE_URL", "http://5.45.126.118:5000/shopify")
...
def _build_entities(html_text, emoji_ids=None):
    ...
    if emoji_ids:
        placeholders = [i for i, ch in enumerate(text) if ch == PE]

        count = min(len(placeholders), len(emoji_ids))

        for i in range(count):
            entities.append(MessageEntityCustomEmoji(
                offset=placeholders[i],
                length=1,
                document_id=emoji_ids[i]
            ))
...
async def start(event):
    ...
    try:
        ...
        if access_type in ["premium_private", "premium_group"]:
            status_line = f"{PE} <b>STATUS</b> тФБ Premium {PE} (<code>{get_cc_limit(access_type, event.sender_id)}</code> CCs)"
            status_emojis = [CE["star"], CE["crown"]]
        else:
            status_line = f"<b>STATUS</b> тФБ Free Tier тФБ (<code>{get_cc_limit(access_type, event.sender_id)}</code> CCs)"
            status_emojis = []

        text = f"""{PE} <b><i>Checker Commands</i></b>
|   {PE} <code>/sh</code> тФБ Single CC check
|   {PE} <code>/msh</code> тФБ Multi CC from text
|   {PE} <code>/mtxt</code> тФБ Mass CC from <code>.txt</code> file
|   {PE} <code>/ran</code> тФБ Mass CC with random sites

{PE} <b><i>Management</i></b>
|   {PE} <code>/add</code> тФБ Add site(s) to your DB
|   {PE} <code>/rm</code> тФБ Remove site(s) from DB
|   {PE} <code>/check</code> тФБ Test saved sites
|   {PE} <code>/info</code> тФБ Your profile &amp; stats
|   {PE} <code>/redeem</code> тФБ Redeem a premium key

{PE} <b>Proxy</b> (Private Only)
|   {PE} <code>/addpxy</code> тФБ Add proxy (max 10)
|   {PE} <code>/proxy</code> тФБ View saved proxies
|   {PE} <code>/chkpxy</code> тФБ Test proxy status
|   {PE} <code>/rmpxy</code> тФБ Remove proxy

{status_line}"""

        kb = [
            [pbtn("ЁЯТО Plans", data="plans"), pbtn("ЁЯУЮ Support", url="https://t.me/Mod_By_Kamal")],
        ]
        start_emojis = [
            CE["bolt"],
...
async def plans_callback(event):
    plans_text = f"""{PE} <b><i>PREMIUM PLANS</i></b> {PE}
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ

{PE} <b>STARTER</b> тФБ 7 Days
тЬЕ 500 CCs тЬЕ Private Chat тЬЕ Proxy

{PE} <b>BASIC</b> тФБ 30 Days
тЬЕ 500 CCs тЬЕ Private Chat тЬЕ Proxy

{PE} <b>LEGEND</b> тФБ Lifetime
тЬЕ 500 CCs тЬЕ Private Chat тЬЕ Proxy
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
{PE} Contact @Mod_By_Kamal to purchase"""

    kb = [[pbtn("ЁЯТ░ Buy Now", url="https://t.me/Mod_By_Kamal"), pbtn("ЁЯФЩ Back", data="back_start")]]
    plans_emojis = [CE["gem"], CE["gem"], CE["star"], CE["crown"], CE["fire"], CE["gift"]]
    await event.answer()
    msg = await event.get_message()
    await styled_edit(msg, plans_text, buttons=kb, emoji_ids=plans_emojis)
...
{status_line}"""
    ...
    await event.answer()
    msg = await event.get_message()
    await styled_edit(msg, text, buttons=kb, emoji_ids=start_emojis)
...
async def auth_user(event):
    ...
    try:
        ...
        try:
            await styled_send(user_id, f"{PE} <b>Premium Activated</b>\nтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ\nтФБ Duration: {days} days\nтФБ Limit: 500 CCs\nтФБ Private chat unlocked", emoji_ids=[CE["crown"]])
        except Exception:
            pass
    except ValueError:
...
async def add_proxy_cmd(event):
    ...
    try:
        ...
        if not is_working:
            await styled_edit(testing_msg, f"{PE} <b>Proxy not working</b>\n\nтФБ {result}", emoji_ids=[CE["cross"]])
            return

        await add_proxy_db(event.sender_id, proxy_data)
        new_count = current_count + 1

        auth_display = f"тФБ Auth: {proxy_data['username']}" if proxy_data.get('username') else "тФБ No Auth"
        await styled_edit(testing_msg, f"{PE} <b>Proxy Added</b>\nтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ\nтФБ IP: {result}\nтФБ Proxy: {proxy_data['ip']}:{proxy_data['port']}\nтФБ Type: {proxy_type_display}\n{auth_display}\nтФБ Total: {new_count}/10", emoji_ids=[CE["check"]])
...
async def remove_proxy_cmd(event):
    ...
    try:
        ...
        try:
            ...
            await styled_reply(event, f"{PE} Proxy removed\nтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ\nтФБ {removed_proxy['ip']}:{removed_proxy['port']}\nтФБ Remaining: {remaining}", emoji_ids=[CE["check"]])
    ...
    try:
        ...
        for idx, proxy_data in enumerate(user_proxies, 1):
            ...
            if proxy_data.get('username'):
                auth_info = f" тФБ {proxy_data['username']}"
            proxy_lines.append(f"<code>{idx}.</code> {PE} {proxy_type} тФБ {proxy_data['ip']}:{proxy_data['port']}{auth_info}")
...
async def check_proxy_cmd(event):
    ...
    try:
        ...
        for idx, proxy_data in enumerate(user_proxies, 1):
            ...
            if is_working:
                working.append(f"{PE} <code>{idx}.</code> {proxy_type} тФБ {display} тФБ {result_ip}")
                working_emojis.append(CE["tick"])
            else:
                dead.append(f"{PE} <code>{idx}.</code> {proxy_type} тФБ {display}")
        ...
        result_text += f"тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ\n{PE} {len(working)} working тФБ {PE} {len(dead)} dead тФБ {PE} {len(user_proxies)} total"
...
async def sh(event):
    ...
    if access_type == "banned":
        ...
        return await styled_reply(event, ban_text, emoji_ids=ban_emojis)
    if not can_access:
        kb = [[pbtn("ЁЯФЧ Join Group", url="https://t.me/+pNplrRLrEGY5NTU0")]]
        await styled_reply(event, "<b>ACCESS DENIED</b>\n\nUse in group for free or get premium.\nтФБ Contact @Mod_By_Kamal", kb, emoji_ids=[CE["stop"]])
        return
    await ensure_user(event.sender_id)
...
async def process_sh_card(event, access_type):
    ...
    async def animate_loading():
        emojis = ["тП│", "тП│тП│", "тП│тП│тП│", "тП│тП│тП│тП│", "тП│тП│тП│тП│тП│"]
        i = 0
        while True:
            try:
    ...
    try:
        ...
        if "cloudflare" in response_text_lower:
            status_header = f"{PE} тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ {PE}"
            header_emojis = [CE["warn"], CE["warn"]]
            res["Response"] = "Cloudflare detected тФБ change site or retry"
            is_charged = False
        elif status == "Error" or status == "SiteError":
        ...
        msg = f"""{status_header}
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
Card тФБ <code>{card}</code>
Gateway тФБ {res.get('Gateway', 'Unknown')}
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
Response тФБ {res.get('Response')}
Price тФБ {res.get('Price')}
Card тФБ {site_index}
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
...
async def msh(event):
    ...
    kb = [
        [pbtn("тЬЕ Yes тФБ Charged + Approved", f"msh_pref:yes:{user_id}")],
        [pbtn("тЭМ No тФБ Only Charged", f"msh_pref:no:{user_id}")]
    ]

    pref_msg = await styled_reply(event,
        f"{PE} <b>FILTER MODE</b>\nтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ\n"
        "<i>тЬЕ Yes: Charged + Approved cards</i>\n"
        "тЭМ <i>No: Only Charged cards</i>",
...
async def process_msh_cards(event, cards, sites, send_approved=True):
    ...
    sent_msg = await styled_reply(event, f"<pre>{PE} Processing тФБ {len(cards)} cards тФБ {mode_text}</pre>", emoji_ids=[CE["chart"]])
        ...
        for batch_start in range(0, len(cards), BATCH_SIZE):
            ...
            for i, (card, result) in enumerate(zip(batch, results)):
                ...
                if "cloudflare" in response_text_lower:
                    status_header = f"{PE} тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ {PE}"
                    header_emojis = [CE["warn"], CE["warn"]]
                    result["Response"] = "Cloudflare detected тФБ change site or retry"
                    is_charged = False
...
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
Card тФБ <code>{card}</code>
Gateway тФБ {result.get('Gateway', 'Unknown')}
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
Response тФБ {result.get('Response')}
Price тФБ {result.get('Price')}
Card тФБ {site_info[1]}
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
<pre>BIN: {brand} | {bin_type} | {level}
Bank: {bank}
Country: {country} {flag}</pre>
...
async def mtxt(event):
    ...
    if user_id in ACTIVE_MTXT_PROCESSES:
        return await styled_reply(event, f"<pre>{PE} Already processing тФБ wait for completion</pre>", emoji_ids=[CE["star"]])
    try:
        ...
        kb = [
            [pbtn("тЬЕ Yes тФБ Charged + Approved", f"mtxt_pref:yes:{user_id}")],
            [pbtn("тЭМ No тФБ Only Charged", f"mtxt_pref:no:{user_id}")]
        ]
...
async def process_mtxt_cards(event, cards, local_sites, send_approved=True):
    ...
    is_private = event.chat.id == event.sender_id
    mode_text = "Charged + Approved" if send_approved else "Only Charged"
    status_msg = await styled_reply(event, f"<pre>Processing тФБ {mode_text}</pre>")
    current_site_index = 0
    BATCH_SIZE = 20
    last_card_display = ""
...
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
{PE} Charged тФБ {charged}
{PE} Approved тФБ {approved}
{PE} Declined тФБ {declined}
{PE} Errors тФБ {errors}
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
{PE} Checked тФБ {checked}/{total}
"""
                final_kb = [
                    [pbtn(f"ЁЯТ░ Charged тФБ {charged}", "none")],
                    [pbtn(f"тЬЕ Approved тФБ {approved}", "none")],
                    [pbtn(f"ЁЯЫС Stopped тФБ {checked}/{total}", "none")]
                ]
                try:
...
"""
                        final_kb = [
                            [pbtn(f"ЁЯТ░ Charged тФБ {charged}", "none")],
                            [pbtn(f"тЬЕ Approved тФБ {approved}", "none")],
                            [pbtn(f"ЁЯТА Sites Dead тФБ {checked}/{total}", "none")]
                        ]
                        try:
...
"""
            ...
            kb = [
                [pbtn(f"ЁЯТ│ {last_card_display}", "none")],
                [pbtn(f"ЁЯУЭ {last_response_display}", "none")],
                [pbtn(f"ЁЯМР Site тФБ {last_site_display}", "none")],
                [pbtn(f"ЁЯТ░ Charged тФБ {charged}", "none")],
                [pbtn(f"тЬЕ Approved тФБ {approved}", "none")],
                [pbtn(f"тЭМ Declined тФБ {declined}", "none")],
                [pbtn(f"тЪая╕П Errors тФБ {errors}", "none")],
                [pbtn(f"ЁЯУК {checked}/{total}", "none")],
                [pbtn("ЁЯЫС Stop", f"stop_mtxt:{user_id}")]
            ]
            try:
        ...
        final_caption = f"""{PE} <b>тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ</b>
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
{PE} Charged тФБ {charged}
{PE} Approved тФБ {approved}
{PE} Declined тФБ {declined}
...
"""
        final_kb = [
            [pbtn(f"ЁЯТ░ Charged тФБ {charged}", "none")],
            [pbtn(f"тЬЕ Approved тФБ {approved}", "none")],
            [pbtn(f"ЁЯУК Total тФБ {total}", "none")],
            [pbtn(f"ЁЯУК Checked тФБ {checked}/{total}", "none")]
        ]
        try:
...
async def info(event):
    ...
    info_text = f"""{PE} <b>PROFILE</b>
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
Card тФБ {full_name}
Card тФБ {username}
Card тФБ <code>{user_id}</code>
??Price тФБ {premium_status}
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
{PE} <b>SITES</b> ({len(user_sites)}):
{sites_text}
"""
...
async def stats(event):
    ...
    try:
        ...
        stats_content += f"тФБ Generated: {current_time}\n\n"

        stats_content += "? USER STATISTICS\n"
        stats_content += "?" * 30 + "\n"
        stats_content += f"тФБ Total Unique Users: {total_users}\n"
        stats_content += f"тФБ Premium Users: {total_premium}\n"
        stats_content += f"тФБ Free Users: {total_free}\n\n"
        ...
        stats_content += "\n? SITES STATISTICS\n"
        stats_content += "?" * 30 + "\n"
        stats_content += f"тФБ Total Sites Added: {total_sites}\n"
        stats_content += f"тФБ Users with Sites: {users_with_sites}\n"
        ...
        stats_content += f"\n KEYS STATISTICS\n"
        stats_content += "?" * 30 + "\n"
        stats_content += f"тФБ Total Keys Generated: {total_keys}\n"
        stats_content += f"тФБ Used Keys: {used_keys}\n"
        stats_content += f"тФБ Unused Keys: {unused_keys}\n"
        ...
        stats_content += f"\n ADMIN STATISTICS\n"
        stats_content += "?" * 30 + "\n"
        stats_content += f"тФБ Total Admins: {len(ADMIN_ID)}\n"
        stats_content += f"Admin IDs: {', '.join(map(str, ADMIN_ID))}\n"

        stats_content += f"\n CARD STATISTICS\n"
        stats_content += "?" * 30 + "\n"
        stats_content += f"тФБ Total Processed Cards: {total_cards}\n"
        stats_content += f"тФБ Approved Cards: {approved_cards}\n"
        stats_content += f"тФБ Charged Cards: {charged_cards}\n"
...
async def ranfor(event):
    ...
    try:
        ...
        kb = [
            [pbtn("тЬЕ Yes тФБ Charged + Approved", f"ran_pref:yes:{user_id}")],
            [pbtn("тЭМ No тФБ Only Charged", f"ran_pref:no:{user_id}")]
        ]
...
async def process_ranfor_cards(event, cards, global_sites, send_approved=True):
    ...
    is_private = event.chat.id == event.sender_id
    mode_text = "Charged + Approved" if send_approved else "Only Charged"
    status_msg = await styled_reply(event, f"<pre>{PE} Processing тФБ {mode_text}</pre>", emoji_ids=[CE["joker"]])
    BATCH_SIZE = 20
    last_card_display = ""
    last_response_display = ""
...
"""
            ...
            kb = [
                ...
                [pbtn(f"тЪая╕П Errors тФБ {errors}", "none")],
                [pbtn(f"ЁЯУК {checked}/{total}", "none")],
                [pbtn("ЁЯЫС Stop", f"stop_ranfor:{user_id}")]
            ]
            try:
                await styled_edit(status_msg, f"<pre>{PE} Processing batches of 20...</pre>", buttons=kb, emoji_ids=[CE["star"]])
            except Exception:
...
async def check_sites(event):
    ...
    if not check_text:
        kb = [
            [pbtn("ЁЯФН Check My DB Sites", "check_db_sites")]
        ]
...
And if you want to check your DB sites and add working &amp; remove not working sites, click below button:"""
    ...
    if len(sites_to_check) > 10:
        ...
        await styled_reply(event, f"<pre>{PE} Found {total_sites_found} sites тФБ checking first 10</pre>", emoji_ids=[CE["warn"]])
...
{PE} Working тФБ {len(working_sites)}
{PE} Dead тФБ {len(dead_sites)}
{PE} Checked тФБ {checked}/{total_sites}"""
                    try:
                ...
                status_text = (
                    ...
                    f"тФБ Current: {site}\n"
                    f"тФБ Status: {result['status'].upper()}\n"
                    f"тФБ Price: {result['price']}</pre>"
                )
...
"""
        if working_sites:
            ...
            for idx, site_data in enumerate(working_sites, 1):
                final_text += f"{PE} <code>{site_data['site']}</code> тФБ {site_data['price']}\n"
        ...
        if working_sites:
            ...
            buttons.append([pbtn("тЬЕ Add Working Sites to DB", f"add_working:{event.sender_id}")])
...
{PE} Checked тФБ {checked}/{total_sites}"""
                ...
                status_text = (
                    ...
                    f"{PE} Dead: {dead_count}\n\n"
                    f"тФБ Current: {site}\n"
                    f"тФБ Status: {result['status'].upper()}</pre>"
                )
...
тФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБтФБ
{PE} Working тФБ {len(working_sites)}
{PE} Dead (Removed) тФБ {len(dead_sites)}
